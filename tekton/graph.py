...
class NetworkGraph(nx.DiGraph):
    ...
    def is_peer(self, node):
        ...
        if not self.has_node(node):
            return False
        return self.nodes[node][VERTEX_TYPE] == VERTEXTYPE.PEER
    ...
    def is_local_router(self, node):
        ...
        if not self.has_node(node):
            return False
        return self.nodes[node][VERTEX_TYPE] == VERTEXTYPE.ROUTER

    def is_network(self, node):
        """Node is a just a subnet (not a router)"""
        if not self.has_node(node):
            return False
        return self.nodes[node][VERTEX_TYPE] == VERTEXTYPE.NETWORK
...
#        assert self.is_local_router(u), "Source '%s' is not a router" % u
    ...
    def get_loopback_interfaces(self, node):
        ...
        :return: a dict
        """
        if 'loopbacks' not in self.nodes[node]:
            self.nodes[node]['loopbacks'] = {}
        return self.nodes[node]['loopbacks']
    ...
    def get_loopback_addr(self, node, loopback):
        ...
        """
        addr = self.nodes[node].get('loopbacks', {}).get(loopback, {}).get('addr', None)
    ...
    def set_loopback_description(self, node, loopback, description):
        ...
        :return: None
        """
        self.nodes[node]['loopbacks'][loopback]['description'] = description
    ...
    def get_loopback_description(self, node, loopback):
        ...
        :return: text or none
        """
        return self.nodes[node]['loopbacks'][loopback].get('description', None)

    def get_ifaces(self, node):
        if 'ifaces' not in self.nodes[node]:
            self.nodes[node]['ifaces'] = {}
        return self.nodes[node]['ifaces']
    ...
    def get_static_routes(self, node):
        ...
        assert self.is_router(node)
        if 'static' not in self.nodes[node]:
            self.nodes[node]['static'] = {}
        return self.nodes[node]['static']
    ...
    def add_static_route(self, node, prefix, next_hop):
        ...
        if is_empty(attrs):
            self.nodes[node]['static'] = {}
        self.nodes[node]['static'][prefix] = next_hop

    def get_acls(self, node):
        """Return a dict of ACLs"""
        assert self.is_router(node)
        if 'acls' not in self.nodes[node]:
            self.nodes[node]['acls'] = {}
        return self.nodes[node]['acls']
    ...
    def add_acl(self, node, interface, inout, acl_number=None):
        ...
        if is_empty(attrs):
            self.nodes[node]['acls'] ={}
        if interface not in self.nodes[node]['acls']:
            self.nodes[node]['acls'][interface] = {}
        if not acl_number:
            if not self.nodes[node]['acls'][interface]:
                acl_number = random.randint(0,20)
            else:
                max_num = max(self.nodes[node]['acls'][interface], key=self.nodes[node]['acls'][interface].get)
                acl_number = max_num + 1
        self.nodes[node]['acls'][interface][acl_number] = [inout]
        return acl_number
    ...
    def add_acl_entry(self, node, interface, acl_number, item):
        ...
        """
        if item not in self.nodes[node]['acls'][interface][acl_number]:
            self.nodes[node]['acls'][interface][acl_number].append(item)


    def set_acls_empty(self, node):
        self.nodes[node]['acls'] = VALUENOTSET
    ...
    def set_static_routes_empty(self, node):
        ...
        """
        self.nodes[node]['static'] = VALUENOTSET
    ...
    def enable_ospf(self, node, process_id=100):
        ...
        self.nodes[node]['ospf'] = dict(process_id=process_id, networks={})
    ...
    def get_ospf_process_id(self, node):
        ...
        return self.nodes[node]['ospf']['process_id']
    ...
    def is_ospf_enabled(self, node):
        ...
        if not self.is_local_router(node):
            return False
        return 'ospf' in self.nodes[node]
    ...
    def get_ospf_networks(self, node):
        ...
        err1 = "OSPF is not enabled on router: {}".format(node)
        assert self.is_ospf_enabled(node), err1
        return self.nodes[node]['ospf']['networks']
    ...
    def get_bgp_attrs(self, node):
        ...
        if 'bgp' not in self.nodes[node]:
            self.nodes[node]['bgp'] = {'asnum': None,
                                      'neighbors': {},
                                      'announces': {}}
        return self.nodes[node]['bgp']
    ...
    def get_bgp_communities_list(self, node):
        ...
        if comm_list is None:
            self.nodes[node]['bgp']['communities-list'] = {}
            comm_list = self.nodes[node]['bgp']['communities-list']
        return comm_list
    ...
    def get_as_path_list(self, node):
        ...
        if as_paths_list is None:
            self.nodes[node]['bgp']['as-path-list'] = {}
            as_paths_list = self.nodes[node]['bgp']['as-path-list']
    ...
    def get_route_maps(self, node):
        ...
        assert self.is_router(node)
        maps = self.nodes[node].get('routemaps', None)
        if not maps:
            self.nodes[node]['routemaps'] = {}
            maps = self.nodes[node]['routemaps']
        return maps
    ...
    def get_ip_preflix_lists(self, node):
        ...
        if 'ip_prefix_lists' not in self.nodes[node]:
            self.nodes[node]['ip_prefix_lists'] = {}
        return self.nodes[node]['ip_prefix_lists']
    ...
    def get_print_graph(self):
        ...
        for node, attrs in self.nodes(data=True):
            ...
            if asnum:
                graph.nodes[node]['bgp_asnum'] = asnum
                label += "\nAS Num: %s" % asnum
            graph.nodes[node]['label'] = label
            graph.nodes[node][VERTEX_TYPE] = vtype
    ...
    def get_simplified_graph(self):
        ...
        for node, attrs in self.nodes(data=True):
            graph.nodes[node]['ifaces'] = self.get_ifaces(node)
            graph.nodes[node]['dyn'] = self.nodes[node]['dyn']
            graph.nodes[node]['loopbacks'] = self.nodes[node]['loopbacks']
        for src, dst, attrs in self.edges(data=True):
